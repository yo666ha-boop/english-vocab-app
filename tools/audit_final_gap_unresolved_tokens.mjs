...
function matching(src,start,open,close){let d=0,q=null,e=false,l=false,b=false;for(let i=start;i<src.length;i++){const c=src[i],n=src[i+1];if(l){if(c==='\n')l=false;continue;}if(b){if(c==='*'&&n==='/'){b=false;i++;}continue;}if(q){if(e){e=false;continue;}if(c==='\\'){e=true;continue;}if(c===q)q=null;continue;}if(c==='/'&&n==='/'){l=true;i++;continue;}if(c==='/'&&n==='*'){b=true;i++;continue;}if(c==='"'||c==="'"||c==='`'){q=c;continue;}if(c===open)d++;else if(c===close){d--;if(d===0)return i;}}return -1;}
function subjectConfig(html){const m=/\bconst\s+subjectConfig\s*=\s*/.exec(html);if(!m)throw new Error('subjectConfig not found');let s=m.index+m[0].length;while(/\s/.test(html[s]||''))s++;const e=matching(html,s,'{','}');return vm.runInNewContext(`(${html.slice(s,e+1)})`,Object.create(null),{timeout:5000});}
function rootData(html){const m=/\b(?:const|let|var)\s+DATA\s*=\s*/g.exec(html);if(!m)throw new Error('root DATA not found');let s=m.index+m[0].length;while(/\s/.test(html[s]||''))s++;const e=matching(html,s,'[',']');if(e<0)throw new Error('root DATA end not found');return vm.runInNewContext(`(${html.slice(s,e+1)})`,Object.create(null),{timeout:10000});}
...
const lexAll=new Set();
for(const r of rows)for(const h of ['英語','検索用基本形','変化形・別表記'])for(const t of toks(r[idx[h]])){lexAll.add(t);for(const l of lemmas(t))lexAll.add(l);}
const elementary=new Set();
for(const row of rootData(fs.readFileSync(ROOT,'utf8')).filter(x=>x&&x.dataset==='elementary'))for(const t of toks(row.english)){elementary.add(t);for(const l of lemmas(t))elementary.add(l);}
function cumulativeLexicon(book,grade){const max=gradeNum(grade),out=new Set();for(const r of rows){if(String(r[idx['教科書']]||'').trim()!==book)continue;const gn=gradeNum(r[idx['学年']]);if(!gn||gn>max)continue;for(const h of ['英語','検索用基本形','変化形・別表記'])for(const t of toks(r[idx[h]])){out.add(t);for(const l of lemmas(t))out.add(l);}}return out;}
const cumulativeCache=new Map();
function resolvedByLexicon(t,book,grade){
  if(grammarCore.has(t)||safeNames.has(t)||safeAcronyms.has(t))return {ok:true,kind:'core_or_name'};
  for(const l of lemmas(t))if(elementary.has(l))return {ok:true,kind:'elementary',lemma:l};
  const ck=`${book}/${grade}`;if(!cumulativeCache.has(ck))cumulativeCache.set(ck,cumulativeLexicon(book,grade));const local=cumulativeCache.get(ck);
  for(const l of lemmas(t))if(local.has(l))return {ok:true,kind:'cumulative_textbook_lexicon',lemma:l};
  for(const l of lemmas(t))if(lexAll.has(l))return {ok:false,kind:'other_textbook_or_later_grade',lemma:l};
  return {ok:false,kind:'globally_absent'};
}
const records=[];
for(const gap of gapRows){
  ...
  for(const item of unresolved){
    ...
    for(const t of toks(`${item.q||''} ${item.a||''}`)){
      ...
      if(r.ok)continue;
      const kk=`${t}\t${r.kind}\t${r.lemma||''}`;if(seen.has(kk))continue;seen.add(kk);
      const prev=tokenStats.get(kk)||{token:t,kind:r.kind,lemma:r.lemma||null,item_count:0,sample_ids:[],sample_texts:[]};
      prev.item_count++;if(prev.sample_ids.length<5)prev.sample_ids.push(item.id);if(prev.sample_texts.length<3)prev.sample_texts.push(`${item.q||''} || ${item.a||''}`);tokenStats.set(kk,prev);
    }
  }
  const tokens=[...tokenStats.values()].sort((a,b)=>b.item_count-a.item_count||a.token.localeCompare(b.token));
  const unresolvedWithoutUnknown=unresolved.filter(item=>toks(`${item.q||''} ${item.a||''}`).every(t=>resolvedByLexicon(t,gap.textbook,gap.grade).ok));
  records.push({grade:gap.grade,textbook:gap.textbook,section:gap.section,category:gap.category,runtime_mapped_categories:mapped,unresolved_items:unresolved.length,unresolved_items_with_any_unknown_token:unresolved.length-unresolvedWithoutUnknown.length,unresolved_items_with_all_tokens_resolvable_by_current_classifier:unresolvedWithoutUnknown.length,all_tokens_resolvable_sample_ids:unresolvedWithoutUnknown.slice(0,30).map(x=>x.id),top_unknown_tokens:tokens.slice(0,80),globally_absent_top:tokens.filter(x=>x.kind==='globally_absent').slice(0,40),other_textbook_or_later_top:tokens.filter(x=>x.kind==='other_textbook_or_later_grade').slice(0,40)});
}
const aggregate=new Map();for(const r of records)for(const t of r.top_unknown_tokens){const kk=`${t.token}\t${t.kind}\t${t.lemma||''}`;const p=aggregate.get(kk)||{token:t.token,kind:t.kind,lemma:t.lemma||null,item_count_sum:0,scopes:[]};p.item_count_sum+=t.item_count;p.scopes.push(`${r.grade}/${r.textbook}/${r.category}`);aggregate.set(kk,p);}
const out={generated_at:new Date().toISOString(),source_gap_file:GAPS,design:'Token-level classifier for final under50 passMeta=-1 items using elementary plus cumulative textbook vocabulary through the selected grade. Diagnostic only; no whitelist and no passMeta mutation.',records,aggregate_top:[...aggregate.values()].sort((a,b)=>b.item_count_sum-a.item_count_sum||a.token.localeCompare(b.token)).slice(0,120)};
fs.writeFileSync(OUT,JSON.stringify(out,null,2)+'\n');
console.log(JSON.stringify(records.map(r=>({scope:`${r.grade}/${r.textbook}/${r.category}`,unresolved:r.unresolved_items,all_resolvable:r.unresolved_items_with_all_tokens_resolvable_by_current_classifier,top:r.top_unknown_tokens.slice(0,12).map(x=>[x.token,x.kind,x.lemma,x.item_count])})),null,2));
