...
const EXPECTED_BYTES=3812209;
const EXPECTED_SHA256='83921d1bb9b0ed3028d1151c138326e7698278906e6d01180bc1fb1f6b2044a0';
...
export function validateHtml(html,{requireExactArtifact=true,bytes=null,sha=null}={}){
  if(requireExactArtifact){
    assert(bytes===EXPECTED_BYTES,`public v2 bytes mismatch: ${bytes}`);
    assert(sha===EXPECTED_SHA256,`public v2 SHA256 mismatch: ${sha}`);
  }
  ...
  const fallbackTag='<details id="legacyPhotoAnalysisFallback" style="margin-top:16px">';
  const fallbackAt=html.indexOf(fallbackTag);
  const fallbackCloseAt=html.indexOf('</details>',fallbackAt);
  assert(primaryAt>=0,'primary My GPT CTA missing or not primary-styled');
  assert(fallbackAt>=0&&fallbackCloseAt>fallbackAt,'legacy fallback details missing');
  assert(primaryAt<fallbackAt,'My GPT CTA must precede legacy fallback');
  assert(!fallbackTag.includes(' open'), 'legacy fallback must be collapsed by default');
  ...
  const fallback=html.slice(fallbackAt,fallbackCloseAt+'</details>'.length);
  ...
  assert(qbClose-qbAt>1000000,'qb-data payload unexpectedly small');
...
function selfTest(){
  ...
  const runtime='<script id="mikami-answer-gpt-handoff-v1">const x=1; [\'chatgpt.com\', \'chat.openai.com\'].includes(url.hostname); /^\\/g\\/[A-Za-z0-9_-]+/.test(url.pathname); question_ids: ids; source_mode: sourceMode(); usage: \'答案写真はMy GPTへ直接添付。問題IDが見える場合はKnowledgeと完全一致で照合する。\';</script>';
  ...
  console.log('PASS_MIKAMI_PROBLEM_APP_UI_CONTRACT_SELF_TEST');
