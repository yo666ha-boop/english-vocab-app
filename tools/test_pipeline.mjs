...
const dir=fs.mkdtempSync(path.join(os.tmpdir(),'mikami-e2e-'));
const input=path.join(dir,'canonical.html'), output=path.join(dir,'final.html');
const cases=[
...
const shell=`<!doctype html><html><body><script id="qb-data" type="application/json">${JSON.stringify(qb)}</script><script id="meta-data" type="application/json">${JSON.stringify(meta)}</script><script>const subjectConfig={'英語':{stagesByGrade:{'中1':['人称代名詞','一般動詞（現在形）'],'中2':['不定詞①','動名詞','比較'],'中3':['be動詞と一般動詞（現在形）','be動詞と一般動詞（過去形）','現在完了形（完了・経験）','関係代名詞']},stageMap:{'中1':{'人称代名詞':['人称代名詞'],'一般動詞（現在形）':['一般動詞（現在形）']},'中2':{'不定詞①':['不定詞①'],'動名詞':['動名詞'],'比較':['比較']},'中3':{'be動詞と一般動詞（現在形）':['be動詞と一般動詞（現在形）'],'be動詞と一般動詞（過去形）':['be動詞と一般動詞（過去形）'],'現在完了形（完了・経験）':['現在完了形（完了・経験）'],'関係代名詞':['関係代名詞']}}}};function selectedCategories(){return [];}function selectedValues(){return [];}function currentSubject(){return '英語';}function currentGrade(){return '中3';}function currentTextbook(){return 'ニューホライズン';}function currentSectionIndex(){return 99;}function useVocabGate(){return true;}${passesVocab}${baseFiltered}</script></body></html>`;
fs.writeFileSync(input,shell);
execFileSync(process.execPath,['tools/run_mikami_pipeline.mjs',input,output],{stdio:'pipe'});
...
for(const [id,want] of Object.entries(checks))for(const [k,v] of Object.entries(want))if(byId[id]?.[k]!==v)throw new Error(`${id}.${k}: expected ${v} got ${byId[id]?.[k]}`);
if(!out.includes('minIdx <= 0) return false'))throw new Error('0/-1 fail-closed vocab gate missing');
if(!out.includes('passesPrereqGrammar(item)')||!out.includes('passesQualityGate(item)'))throw new Error('runtime gates missing');
console.log('Mikami end-to-end repair pipeline: OK');
