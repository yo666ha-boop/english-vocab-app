...
function validate(m){
  ...
  hashes.push(a.rendered_pdf_sha256);
  const r=m.runtime_results||{};
  assert(validSha(r.results_json_sha256),'runtime results SHA invalid');
  hashes.push(r.results_json_sha256);
  assert(new Set(hashes).size===hashes.length,'evidence SHA collision detected');
  return {status:'PASS',photo_cases:4,evidence_hashes:hashes.length,registration_kit_bound:true,a4_pages:a.page_count};
}
function synthetic(){
  ...
  return {
    ...
    custom_gpt:{gpt_url:'https://chatgpt.com/g/g-abc123/mikami',registration_verified_at:'2026-08-20T16:40:00+09:00',registration_evidence_sha256:h(1)},
    photo_cases:EXPECTED.case_ids.map((id,i)=>({id,photo_sha256:h(10+i*2),response_transcript_sha256:h(11+i*2),executed_at:`2026-08-20T16:4${i}:00+09:00`})),
    ...
    runtime_results:{results_json_sha256:h(31)}
  };
}
const args=process.argv.slice(2);
try{
  if(args.includes('--self-test')){
    const good=synthetic(); validate(good);
    for(const mutate of [
      x=>x.custom_gpt.gpt_url='SELF_TEST_ONLY',
      x=>x.registration_kit.knowledge_file_id='wrong',
      x=>x.photo_cases[1].photo_sha256=x.photo_cases[0].photo_sha256,
      x=>x.a4_report.page_count=3
    ]){
      const bad=structuredClone(good); mutate(bad); let rejected=false; try{validate(bad);}catch{rejected=true;} assert(rejected,'self-test negative case accepted');
    }
    console.log('PASS_ANSWER_GPT_EVIDENCE_CHAIN_SELF_TEST');
  } else {
    assert(args[0],'Usage: node tools/validate_mikami_answer_gpt_evidence_chain.mjs <manifest.json>');
    console.log(JSON.stringify(validate(JSON.parse(fs.readFileSync(args[0],'utf8'))),null,2));
  }
