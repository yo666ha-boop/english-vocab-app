...
function buildGrade2PronounFallbacks() {
  ...
  for(const r of rows){
    ...
    out.push(vocabFallbackItem('中2','人称代名詞','空所補充','G2-PRON-OBJ-'+stem,'主格 '+cap+' に対応する目的格を空所に書きなさい。 I often talk with (      ).',r.obj));
    out.push(vocabFallbackItem('中2','人称代名詞','選択','G2-PRON-POSS-'+stem,'This is ( '+r.poss+' / '+r.obj+' / '+r.sub+' ) book. 正しい代名詞を選びなさい。',r.poss));
    out.push(vocabFallbackItem('中2','人称代名詞','間違い直し','G2-PRON-FIX-'+stem,'主格 '+cap+' に対応する目的格を使うように誤りを直しなさい。 I often talk with '+r.wrong+'.','I often talk with '+r.obj+'.'));
  }
  return out;
}
...
function vocabFallbackBank() {
  ...
  const all=buildBePresentFallbacks().concat(buildGrade2ImperativeFallbacks(),buildGrade2PronounFallbacks(),buildGrade2Infinitive2Fallbacks(),buildPastProgressiveFallbacks(),buildBePastFallbacks(),buildWordOrderFallbacks(),buildImperativeFallbacks(),buildPresentVerbFallbacks(),buildPatternOneFallbacks(),buildGrade1ConjunctionFallbacks(),buildPastVerbFallbacks(),buildIrregularPastFallbacks(),buildPastTimeFallbacks(),buildPresentPerfectContinuousFallbacks(),buildThereIsFallbacks(),buildBeVsVerbFallbacks(),buildCombinedThereLookFallbacks(),buildQuestionWordFallbacks(),buildPresentProgressiveFallbacks(),buildFutureFallbacks(),buildPresentPerfectCompletionExperienceFallbacks(),buildGerundFallbacks(),buildConjunctionFallbacks());
  const seen=new Set();
...
function appendVocabFallbacks(originals, categories, types, overrideMode) {
  ...
  counts.clear(); // supplemental fallback cap: originals do not consume the fallback allowance
  const extras=[];
